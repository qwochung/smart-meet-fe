...
export default function WaitingScreen({ roomCode, roomName, status = 'Đang chờ chủ phòng phê duyệt...' }) {
  return (
    <div className="flex min-h-[70vh] items-center justify-center px-4 py-10">
      <div className="w-full max-w-2xl rounded-[2rem] border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/50">
        <div className="flex flex-col items-center text-center">
          ...
          <h1 className="mt-6 text-3xl font-bold tracking-tight text-slate-900">Đang chờ chủ phòng phê duyệt...</h1>
          ...
          <div className="mt-8 grid w-full gap-4 rounded-3xl bg-slate-50 p-5 text-left sm:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-white p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Mã phòng</p>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4">
              ...
              <p className="mt-2 text-lg font-semibold text-slate-900">{roomName || 'Đang chờ tên phòng'}</p>
            </div>
          </div>
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-4 text-sm text-slate-500">
            <span className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-4 py-2">
              ...
              LiveKit sẽ tự động khởi động
            </span>
            <span className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-4 py-2">
              <VideoOff className="h-4 w-4 text-slate-500" />
              Camera sẽ tắt cho đến khi được phê duyệt
            </span>
          </div>
        </div>
      </div>
